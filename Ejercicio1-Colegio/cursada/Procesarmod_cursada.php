...
<script
  ...
  $verificar = "SELECT C.* from cursada C where C.id_alumno = $alumno and C.id_materia = $materia and C.id_cursada != $idc";
  $ver = mysqli_query($con, $verificar);
  $valor = mysqli_fetch_array($ver);
  if ($valor){
    echo$verificar;
    echo"<script>
    alert('La cursada ya se encuentra registrada.');
    window.location.href = 'ListadoCursada.php';
    </script>";
  }
  else{
    ...
    echo"<h1 class='titulo'>Se ha modificad con exito la cursada!</h1>";
  }
